...
int turn_on_whole_system_provnance() {
    __android_log_print(ANDROID_LOG_INFO, "turn_on_whole_system_provnance", "start");
    int rc;
    int fd = open(PROV_ALL_FILE, O_WRONLY);

    __android_log_print(ANDROID_LOG_INFO, "turn_on_whole_system_provnance", "PROV_ALL_FILE open_status: %d\n", fd);

    if (fd < 0) {
        return fd;
    }
    // Set Whole System Provenance to true
    rc = write(fd, "1", 2 * sizeof(char));
    __android_log_print(ANDROID_LOG_INFO, "turn_on_whole_system_provnance", "PROV_ALL_FILE write_status: %d\n", rc);
    ...
    __android_log_print(ANDROID_LOG_INFO, "turn_on_whole_system_provnance", "end");
    return 0;
}

bool get_boolean_whole_system_provnance() {
...
int turn_on_provnance() {
    __android_log_print(ANDROID_LOG_INFO, "turn_on_provnance_capture", "start");
    int rc;
    int fd = open(PROV_ENABLE_FILE, O_WRONLY);

    __android_log_print(ANDROID_LOG_INFO, "turn_on_provnance_capture", "PROV_ENABLE_FILE open_status: %d\n", fd);
    ...
    // Set Whole System Provenance to true
    rc = write(fd, "1", 2 * sizeof(char));
    __android_log_print(ANDROID_LOG_INFO, "turn_on_provnance_capture", "PROV_ENABLE_FILE write_status: %d\n", rc);
    ...
    __android_log_print(ANDROID_LOG_INFO, "turn_on_provnance_capture", "end");
    return 0;
}

int main() {
    // camflow -e true: set enable provenance to true
//    if (!get_boolean_provnance_on()) {
//        int state_enable = turn_on_provnance();
//        __android_log_print(ANDROID_LOG_INFO, "provenance_relay_register", "capture enable: %d\n", state_enable);
//    } else {
//        __android_log_print(ANDROID_LOG_INFO, "provenance_relay_register", "capture is already enabled");
//    }

    // camflow -a true: turn on whole system provenance
//    if (!get_boolean_whole_system_provnance()) {
//        int state_all = turn_on_whole_system_provnance();
//        __android_log_print(ANDROID_LOG_INFO, "provenance_relay_register", "capture all enable: %d\n", state_all);
//    } else {
//        __android_log_print(ANDROID_LOG_INFO, "provenance_relay_register", "capture all is already enabled");
//    }
